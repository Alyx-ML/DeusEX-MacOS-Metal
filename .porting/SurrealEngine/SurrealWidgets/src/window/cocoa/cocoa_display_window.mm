...
InputKey keycode_to_inputkey(unsigned short keycode)
{
    static const std::map<unsigned short, InputKey> keymap =
    {
        ...
        {47, InputKey::Period},
        {50, InputKey::Tilde},
        {54, InputKey::RCommand},
        {64, InputKey::F17},
        {65, InputKey::NumPadPeriod},
        {67, InputKey::GreyStar},
        {69, InputKey::GreyPlus},
        {71, InputKey::NumLock},
        {75, InputKey::GreySlash},
        {76, InputKey::Enter},
        {78, InputKey::GreyMinus},
        {79, InputKey::F18},
        {80, InputKey::F19},
        {81, InputKey::Equals},
        {82, InputKey::NumPad0},
...
- (id)initWithImpl:(CocoaDisplayWindowImpl*)d
{
    ...
    if (self)
    {
        ...
        self.wantsLayer = YES;
        self.layer.contentsScale = impl->renderAPI == RenderAPI::Metal ? 1.0 : [NSScreen mainScreen].backingScaleFactor;
...
- (void)viewDidMoveToWindow
{
    if ([self window])
    {
        ...
        // Vulkan and Metal are driven by the game loop. A second render thread
        // would race the command encoder and drawable.
...
- (void)mouseDown:(NSEvent *)theEvent
{
    if (impl && impl->windowHost)
    {
        ...
        {
            impl->keyState[mouseKey] = true;
            if ([theEvent clickCount] == 2)
                impl->windowHost->OnWindowMouseDoubleclick(Point(p.x, flippedY), mouseKey);
            else
...
{
    if (!impl || !impl->windowHost) return;
    auto update = [&](InputKey key, bool pressed)
    {
        if (impl->keyState[key] == pressed) return;
        impl->keyState[key] = pressed;
        if (pressed) impl->windowHost->OnWindowKeyDown(key);
        else impl->windowHost->OnWindowKeyUp(key);
    };
    NSEventModifierFlags flags = [theEvent modifierFlags];
    update(InputKey::LShift, (flags & NX_DEVICELSHIFTKEYMASK) != 0);
    update(InputKey::RShift, (flags & NX_DEVICERSHIFTKEYMASK) != 0);
    update(InputKey::LControl, (flags & NX_DEVICELCTLKEYMASK) != 0);
    update(InputKey::RControl, (flags & NX_DEVICERCTLKEYMASK) != 0);
    update(InputKey::LCommand, (flags & NX_DEVICELCMDKEYMASK) != 0);
    update(InputKey::RCommand, (flags & NX_DEVICERCMDKEYMASK) != 0);
    update(InputKey::Shift, (flags & NSEventModifierFlagShift) != 0);
    update(InputKey::Ctrl, (flags & NSEventModifierFlagControl) != 0);
    update(InputKey::Alt, (flags & NSEventModifierFlagOption) != 0);
    update(InputKey::CapsLock, (flags & NSEventModifierFlagCapsLock) != 0);
}
...
- (id)initWithImpl:(CocoaDisplayWindowImpl*)d
{
    ...
    {
        impl = d;
        [NSWorkspace.sharedWorkspace.notificationCenter addObserver:self selector:@selector(windowDidResignKey:) name:NSWorkspaceWillSleepNotification object:nil];
        [NSWorkspace.sharedWorkspace.notificationCenter addObserver:self selector:@selector(windowDidBecomeKey:) name:NSWorkspaceDidWakeNotification object:nil];
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(windowDidResignKey:) name:NSApplicationDidResignActiveNotification object:nil];
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(windowDidBecomeKey:) name:NSApplicationDidBecomeActiveNotification object:nil];
    }
    return self;
}
...
- (void)windowDidResignKey:(NSNotification *)notification
{
    ...
    if (impl->windowHost)
    {
        for (auto& state : impl->keyState)
        {
            if (!state.second) continue;
            state.second = false;
            impl->windowHost->OnWindowKeyUp(state.first);
        }
...
{
    if (impl->windowHost && impl->window.isKeyWindow && NSApp.active) impl->windowHost->OnWindowActivated();
}

- (void)windowDidResize:(NSNotification *)notification
{
    if (impl->windowHost) impl->windowHost->OnWindowGeometryChanged();
}

- (void)windowDidChangeScreen:(NSNotification *)notification
{
    if (impl->metalLayer) impl->metalLayer.contentsScale = impl->window.backingScaleFactor;
    impl->updateDrawableSize(impl->window.contentView.bounds.size);
    if (impl->windowHost) {
        impl->windowHost->OnWindowDpiScaleChanged();
        impl->windowHost->OnWindowGeometryChanged();
    }
}

- (void)windowDidChangeBackingProperties:(NSNotification *)notification { [self windowDidChangeScreen:notification]; }
- (void)windowDidEnterFullScreen:(NSNotification *)notification { fprintf(stderr, "macOS native fullscreen: entered\n"); [self windowDidChangeScreen:notification]; }
- (void)windowDidExitFullScreen:(NSNotification *)notification { fprintf(stderr, "macOS native fullscreen: exited\n"); [self windowDidChangeScreen:notification]; }
- (void)windowDidMiniaturize:(NSNotification *)notification { [self windowDidResignKey:notification]; }
- (void)windowDidDeminiaturize:(NSNotification *)notification { [self windowDidBecomeKey:notification]; }
...
{
    if (impl->window && impl->window.isKeyWindow && !impl->cursorLocked)
    {
        // Keep the detached pointer inside the view so AppKit routes mouse events here.
        NSView* view = impl->window.contentView;
        NSPoint center = NSMakePoint(NSMidX(view.bounds), NSMidY(view.bounds));
        center = [impl->window convertPointToScreen:[view convertPoint:center toView:nil]];
        CGWarpMouseCursorPosition(CGPointMake(center.x, NSMaxY(NSScreen.screens.firstObject.frame) - center.y));
