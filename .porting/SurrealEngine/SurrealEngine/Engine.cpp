...
void Engine::Run()
{
	...
	while (!quit)
	{
		...
		// Process focus/wake events before computing simulation time.
		...
		SetPause(!LevelInfo->Pauser().empty() || (!appActive && LevelInfo->NetMode() == 0));
        if (!m_GamePaused) {
            if (EntryLevel) EntryLevelInfo->TimeSeconds() += entryLevelElapsed;
        ...
        if (appActive || LevelInfo->NetMode() != 0) {
            if (EntryLevel) EntryLevel->Tick(entryLevelElapsed, m_GamePaused);
		...
		{
            try { SaveGameToSlot(SaveGameInfo.SaveGameSlot, SaveGameInfo.SaveGameDescription); }
            catch (const std::exception& error) {
                LogMessage("Save failed: " + std::string(error.what()));
                CallEvent(viewport->Actor(), "ClientMessage", {ExpressionValue::StringValue("Save failed; previous save preserved. See game.log for details.")});
            }
...
void Engine::ClientTravel(const std::string& newURL, ETravelType travelType, bool transferItems)
{
	...
	if (packages->IsDeusEx() && url.HasOption("loadgame"))
		url.AddOrReplaceOption("load=" + url.GetOption("loadgame"));
...
void Engine::LoadFromSaveFile(const UnrealURL& url)
{
	...
	{
		slotNum = Convert::to_int32(url.GetOption("load"));
		savefilePackage = packages->LoadSaveSlot(slotNum);
	}

	if (!savefilePackage)
		Exception::Throw("Save slot does not contain a loadable map");
...
void Engine::PossessSavedPlayer()
{
	...
	{
		// Older port saves may lack the persistent flag graph.
		if (!pawnExt->FlagBase())
		{
...
void Engine::SaveGameToSlot(int32_t slotNum, const std::string& saveDescription) const
{
	...
	if (packages->IsDeusEx())
	{
        const auto folder = PackageManager::SaveSlotFolder(slotNum);
        const auto destination = saveFolderPath / folder;
        const auto staging = saveFolderPath / (folder + ".pending");
        const auto backup = saveFolderPath / (folder + ".previous");
        if (fs::exists(staging)) Exception::Throw("Unfinished save exists: " + staging.string());
        fs::create_directory(staging);
        try {
            const auto mapName = Level->package->GetPackageName().ToString();
            LevelPackage->Save(Level, (staging / (mapName + "." + packages->GetSaveExtension())).string());
            // SaveInfo is an independent root in the map package, using its format version.
            auto* info = UObject::Cast<UDXSaveInfo>(LevelPackage->NewObject("MyDeusExSaveInfo", deusExPackage->GetClass("DeusExSaveInfo"), ObjectFlags::NoFlags));
            ...
            if (viewport->Actor()) {
                info->SaveCount() = viewport->Actor()->GetInt("saveCount");
                info->SaveTime() = (int)viewport->Actor()->GetFloat("saveTime");
            }
            LevelPackage->Save(info, (staging / ("SaveInfo." + packages->GetSaveExtension())).string());
            packages->CloseStreams();
...
void Engine::UpdateInput(float timeElapsed)
{
	...
	if (!viewport->Actor() || !appActive)
		return;

#ifdef __APPLE__
    PollMacController(timeElapsed);
#endif
    for (auto& it : activeInputButtons)
        viewport->Actor()->SetByte(it.first, !it.second.empty());
	for (auto& it : activeInputAxes)
	{
        float value = 0, limit = 0;
        for (auto& input : it.second) { value += input.second; limit = std::max(limit, std::abs(input.second)); }
...
void Engine::TickWindow()
{
	...
	{
		if (!appActive || (viewport->bShowWindowsMouse() && viewport->bWindowsMouseAvailable()))
			window->UnlockCursor();
		else
			window->LockCursor();
	}
...
void Engine::OnWindowMouseDown(const Point& pos, EInputKey key)
{
	...
	bool consumed = engine->dxRootWindow && engine->dxRootWindow->OnWindowMouseDown(pos, key);
	if (key == IK_1 || key == IK_LeftMouse || key == IK_Escape || key == IK_Space || key == IK_Q || key == IK_E)
		fprintf(stderr, "INPUT MouseDown key=%d consumed=%d binding=%s\n", (int)key, consumed, keybindings[keynames[key]].c_str());
	if (consumed) return;
...
void Engine::OnWindowMouseUp(const Point& pos, EInputKey key)
{
	...
	bool consumed = engine->dxRootWindow && engine->dxRootWindow->OnWindowMouseUp(pos, key);
	if (key == IK_1 || key == IK_LeftMouse || key == IK_Escape || key == IK_Space || key == IK_Q || key == IK_E)
		fprintf(stderr, "INPUT MouseUp key=%d consumed=%d binding=%s\n", (int)key, consumed, keybindings[keynames[key]].c_str());
	if (consumed)
	{
		ReleaseInput(key);
		return;
...
void Engine::OnWindowKeyDown(EInputKey key)
{
	...
	bool repeat = pressedKeys[key];
	pressedKeys[key] = true;
	if (repeat && (key == IK_Escape || key == IK_Enter)) return;
	bool consumed = engine->dxRootWindow && engine->dxRootWindow->OnWindowKeyDown(key, repeat);
	if (key == IK_1 || key == IK_LeftMouse || key == IK_Escape || key == IK_Space || key == IK_Q || key == IK_E)
		fprintf(stderr, "INPUT KeyDown key=%d consumed=%d binding=%s\n", (int)key, consumed, keybindings[keynames[key]].c_str());
	if (consumed) return;

	if (!repeat) InputEvent(key, IST_Press);
}
...
void Engine::OnWindowKeyUp(EInputKey key)
{
	...
	bool consumed = engine->dxRootWindow && engine->dxRootWindow->OnWindowKeyUp(key);
	if (key == IK_1 || key == IK_LeftMouse || key == IK_Escape || key == IK_Space || key == IK_Q || key == IK_E)
		fprintf(stderr, "INPUT KeyUp key=%d consumed=%d binding=%s\n", (int)key, consumed, keybindings[keynames[key]].c_str());
	if (consumed)
	{
		ReleaseInput(key);
		return;
...
void Engine::OnWindowActivated()
{
	appActive = true;
	lastTime = 0;
	SetPause(LevelInfo && !LevelInfo->Pauser().empty());
}
...
void Engine::OnWindowDeactivated()
{
	...
	ResetInput();
	UnlockCursor();
	if (LevelInfo && LevelInfo->NetMode() == 0) SetPause(true);
}
...
void Engine::ResetInput()
{
    if (viewport && viewport->Actor())
    {
        ...
        for (UProperty* prop : pawn->Class->Properties)
        {
            if (!AllFlags(prop->PropFlags, PropertyFlags::Input)) continue;
            if (prop->ValueType == ExpressionValueType::ValueByte)
                pawn->SetByte(prop->Name, 0);
            else if (prop->ValueType == ExpressionValueType::ValueFloat)
                pawn->SetFloat(prop->Name, 0.0f);
        }
    }
...
void Engine::ReleaseInput(EInputKey key)
{
    for (auto it = activeInputButtons.begin(); it != activeInputButtons.end();) {
        it->second.erase(key);
        if (it->second.empty()) {
            if (viewport && viewport->Actor()) viewport->Actor()->SetByte(it->first, 0);
    ...
    for (auto it = activeInputAxes.begin(); it != activeInputAxes.end();) {
        it->second.erase(key);
        if (it->second.empty()) {
            if (viewport && viewport->Actor()) viewport->Actor()->SetFloat(it->first, 0);
            it = activeInputAxes.erase(it);
        } else ++it;
    }
}

void Engine::UpdateController(const ControllerState& incoming, float elapsed)
{
    ControllerState state = appActive && controllerEnabled ? incoming : ControllerState{};
    bool menu = dxRootWindow && dxRootWindow->IsModalOpen();
    const EInputKey menuKeys[16] = {IK_Enter, IK_Escape, IK_None, IK_None, IK_Left, IK_Right,
        IK_None, IK_None, IK_None, IK_None, IK_None, IK_Escape, IK_Up, IK_Down, IK_Left, IK_Right};
    bool changed = state.device != previousController.device || menu != controllerInMenu;
    if (changed) {
        if (controllerClickHeld && dxRootWindow) dxRootWindow->OnWindowMouseUp({}, IK_LeftMouse);
        controllerCursorMode = controllerClickHeld = false;
        for (int i=0; i<16; ++i) {
            ReleaseInput((EInputKey)(IK_Joy1+i));
            if (controllerInMenu && previousController.buttons[i] && menuKeys[i] != IK_None && dxRootWindow)
                dxRootWindow->OnWindowKeyUp(menuKeys[i]);
            controllerRepeat[i] = 0;
        }
        for (auto key : {IK_JoyX, IK_JoyY, IK_JoyR, IK_JoyU}) ReleaseInput(key);
    }
    if (menu && state.device && !changed && std::isfinite(state.moveX) && std::isfinite(state.moveY)
        && (state.moveX != 0 || state.moveY != 0)) {
        controllerCursorMode = true;
        float dt = std::clamp(elapsed, 0.0f, 0.1f);
        auto speed = [](float axis) { axis = std::clamp(axis, -1.0f, 1.0f); return axis * (0.25f + 0.75f * std::abs(axis)) * 500.0f; };
        dxRootWindow->SetRootCursorPos(dxRootWindow->MouseX() + speed(state.moveX) * dt,
            dxRootWindow->MouseY() - speed(state.moveY) * dt);
    }
    for (int i=0; i<16; ++i) {
        bool before = previousController.buttons[i];
        auto key = (EInputKey)(IK_Joy1+i);
        if (!state.buttons[i] && before) {
            ReleaseInput(key);
            if (menu && menuKeys[i] != IK_None && dxRootWindow) dxRootWindow->OnWindowKeyUp(menuKeys[i]);
        }
        // A held button must be released before it can act after reconnect/menu change.
        bool press = state.device && state.buttons[i] && !before && !changed;
        if (menu) {
            if (i == 0 && controllerClickHeld && !state.buttons[i]) {
                dxRootWindow->OnWindowMouseUp({}, IK_LeftMouse);
                controllerClickHeld = false;
            }
            if (i == 0 && controllerCursorMode) {
                if (press) { controllerClickHeld = true; dxRootWindow->OnWindowMouseDown({}, IK_LeftMouse); }
                if (dxRootWindow->IsModalOpen() != menu) break;
                continue;
            }
            if (i >= 12 && press) controllerCursorMode = false;
            bool repeat = false;
            if (!changed && state.buttons[i] && before && menuKeys[i] >= IK_Left && menuKeys[i] <= IK_Down) {
                controllerRepeat[i] -= elapsed;
                if (controllerRepeat[i] <= 0) { repeat = true; controllerRepeat[i] = 0.12f; }
            } else controllerRepeat[i] = 0.4f;
            if ((press || repeat) && menuKeys[i] != IK_None) dxRootWindow->OnWindowKeyDown(menuKeys[i], repeat);
        } else if (press) InputEvent(key, IST_Press);
        if (dxRootWindow && dxRootWindow->IsModalOpen() != menu) break;
    }
    if (state.device && !menu && !(dxRootWindow && dxRootWindow->IsModalOpen()) && viewport && viewport->Actor()) {
        auto axis = [&](const char* name, EInputKey key, float value, float speed) {
            if (!std::isfinite(value)) value = 0;
            activeInputAxes[name][key] = std::clamp(value, -1.0f, 1.0f) * speed;
        };
        axis("aStrafe", IK_JoyX, state.moveX, 6000);
        axis("aBaseY", IK_JoyY, state.moveY, 6000);
        axis("aBaseX", IK_JoyR, state.lookX, 3000);
        axis("aLookUp", IK_JoyU, state.lookY, 2000);
    }
    previousController = state;
    controllerInMenu = menu;
}

void Engine::InputEvent(EInputKey key, EInputType type, int delta)
{
	if (type == EInputType::IST_Release) ReleaseInput(key);
	if (Frame::RunState != FrameRunState::Running || playingAvi)
		return;
	...
	if (key == IK_1 || key == IK_LeftMouse || key == IK_Escape || key == IK_Space || key == IK_Q || key == IK_E) fprintf(stderr, "INPUT console key=%d type=%d consumed=%d\n", (int)key, (int)type, handled);
...
{
	if (key == IK_1 || key == IK_LeftMouse || key == IK_Escape || key == IK_Space || key == IK_Q || key == IK_E) fprintf(stderr, "INPUT command key=%d command=%s\n", (int)key, commands.c_str());
	for (const std::string& commandline : GetSubcommands(commands))
	{
		...
		if (!args.empty())
		{
			...
			{
				activeInputButtons[args[1]].insert(key);
			}
			else if (command == "axis" && args.size() == 3)
			{
				...
				activeInputAxes[args[1]][key] = speed * delta;
			}
			else
			{
				ExecCommand(args);
...
{
	if (value && !m_GamePaused) ResetInput();
	m_GamePaused = value;
}
