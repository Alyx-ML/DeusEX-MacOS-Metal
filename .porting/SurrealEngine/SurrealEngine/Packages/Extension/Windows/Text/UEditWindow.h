...
private:
	struct EditLine { int start, end; };
	std::vector<EditLine> LayoutLines();
	float TextWidth(int start, int end);
	int CursorLine(const std::vector<EditLine>& lines);
	struct EditState { std::string text; int cursor, start, end; };
	std::vector<EditState> undoStates, redoStates;
	EditState CaptureEdit();
