...
{
	// Keep the apparent UI size independent of the rendering resolution.
	return std::max(engine->viewport->ViewportHeight() / 600.0f * engine->interfaceScale, 0.01f);
}
...
{
    for (auto* cur = parentOwner(); cur; cur = cur->parentOwner())
        if (TryCast<UTabGroupWindow>(cur)) return cur;
    return nullptr;
}
...
UObject* UWindow::MoveFocus(int direction, bool tabGroup)
{
    auto* root = GetRootWindow();
    if (!root) return nullptr;
    auto* current = root->GetKeyboardFocus();
    if (!current) return nullptr;
    auto* modal = Cast<UWindow>(current->GetModalWindow());
    if (!modal) return nullptr;
    auto* group = Cast<UWindow>(current->GetTabGroupWindow());
    auto* scope = tabGroup || !group ? modal : group;
    std::vector<UWindow*> candidates, pending{scope};
    while (!pending.empty()) {
        auto* item = pending.back(); pending.pop_back();
        if (!item->bIsVisible() || !item->bIsSensitive()) continue;
        if (item->bIsSelectable()) candidates.push_back(item);
        for (auto* child = item->lastChild(); child; child = child->prevSibling()) pending.push_back(child);
    }
    if (candidates.empty()) return nullptr;
    // Original tab groups maintain row/column-major traversal orders.
    bool vertical = direction == 2 || direction == -2;
    std::stable_sort(candidates.begin(), candidates.end(), [&](UWindow* a, UWindow* b) {
        float ax, ay, bx, by;
        ConvertCoordinates(a, 0, 0, modal, ax, ay);
        ConvertCoordinates(b, 0, 0, modal, bx, by);
        return vertical ? (ax == bx ? ay < by : ax < bx) : (ay == by ? ax < bx : ay < by);
    });
    bool otherGroup = tabGroup && std::any_of(candidates.begin(), candidates.end(), [&](UWindow* item) { return item->GetTabGroupWindow() != group; });
    auto it = std::find(candidates.begin(), candidates.end(), current);
    int index = it == candidates.end() ? (direction > 0 ? -1 : 0) : (int)(it - candidates.begin());
    for (size_t n = 0; n < candidates.size(); ++n) {
        index = (index + (direction > 0 ? 1 : -1) + (int)candidates.size()) % (int)candidates.size();
        auto* next = candidates[index];
        if (!otherGroup || next->GetTabGroupWindow() != group) {
            root->SetRootFocusWindow(next);
            return next;
        }
    }
    return current;
...
UObject* UWindow::MoveFocusDown() { return MoveFocus(2); }
UObject* UWindow::MoveFocusLeft() { return MoveFocus(-1); }
UObject* UWindow::MoveFocusRight() { return MoveFocus(1); }
UObject* UWindow::MoveFocusUp() { return MoveFocus(-2); }
UObject* UWindow::MoveTabGroupNext() { return MoveFocus(1, true); }
UObject* UWindow::MoveTabGroupPrev() { return MoveFocus(-1, true); }
...
UObject* UWindow::NewChild(UObject* NewClass, std::optional<bool> bShow)
{
	...
	child->bIsSelectable() = TryCast<UButtonWindow>(child) || TryCast<UEditWindow>(child) || TryCast<UListWindow>(child) || TryCast<UScaleWindow>(child);
	if (lastChild())
