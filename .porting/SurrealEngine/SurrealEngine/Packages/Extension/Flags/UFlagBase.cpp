...
{
    // Scan all buckets so original saves and names interned in a different order work.
    for (auto* head : hashTable())
        for (auto* flag = head; flag; flag = flag->nextFlag())
            if (flag->FlagName() == flagName && flag->flagType() == flagType) return flag;
    return nullptr;
}
...
void UFlagBase::DeleteExpiredFlags(int criteria)
{
    ...
    for (int i = 0; i < table.size(); ++i) {
        UFlag** link = &table[i];
        while (*link) {
            if ((*link)->expiration() > 0 && (*link)->expiration() <= criteria) *link = (*link)->nextFlag();
            else link = &(*link)->nextFlag();
        }
    }
}
...
int UFlagBase::CreateIterator(std::optional<uint8_t> flagType)
{
    ...
    for (auto* head : hashTable())
        for (auto* flag = head; flag; flag = flag->nextFlag())
            if (!flagType || flag->flagType() == *flagType)
                flagIt.Flags.emplace_back(flag->FlagName(), flag->flagType());
    FlagIterators[handle] = std::move(flagIt);
	return handle;
}
...
bool UFlagBase::GetNextFlag(int Iterator, NameString& FlagName, uint8_t& flagType)
{
    ...
    if (it == FlagIterators.end()) return false;
    auto& iter = it->second;
    while (iter.Position < iter.Flags.size()) {
        auto item = iter.Flags[iter.Position++];
        if (!CheckFlag(item.first, item.second)) continue;
        FlagName = item.first;
        flagType = item.second;
        return true;
    }
    return false;
}
...
T* UFlagBase::GetOrCreateFlag(const NameString& FlagName, std::optional<bool> bAdd, std::optional<int> expiration, EFlagType flagType, const char* flagClassName)
{
	...
	if (!bAdd || *bAdd)
	{
        ...
        auto table = hashTable();
        int bucket = FlagName.GetCompareIndex() % HashTableSize;
        auto* flag = Cast<T>(engine->packages->GetTransientPackage()->NewObject(FlagName, cls, ObjectFlags::Transient));
        flag->FlagName() = FlagName;
        flag->FlagBase() = this;
        flag->flagType() = (uint8_t)flagType;
        flag->expiration() = newExpiration;
        flag->flagHash() = bucket;
