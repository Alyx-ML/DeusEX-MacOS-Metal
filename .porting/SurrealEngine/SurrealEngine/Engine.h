...
public:
	...
	void ReleaseInput(EInputKey key);
	void ResetInput();
    ...
    std::map<std::string, std::set<EInputKey>> activeInputButtons;
    std::map<std::string, std::map<EInputKey, float>> activeInputAxes;
...
private:
    ...
    bool controllerInMenu = false;
    bool controllerCursorMode = false, controllerClickHeld = false;
    float controllerRepeat[16] = {};
