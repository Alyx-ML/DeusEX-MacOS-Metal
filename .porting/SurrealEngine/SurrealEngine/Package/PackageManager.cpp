...
std::string PackageManager::SaveSlotFolder(int32_t slotNum)
{
    if (slotNum < -1 || slotNum > 9999) Exception::Throw("Invalid save slot");
    if (slotNum == -1) return "QuickSave";
    std::string number = std::to_string(slotNum);
    return "Save" + std::string(4 - number.size(), '0') + number;
}

Package* PackageManager::LoadSaveSlot(int32_t slotNum)
{
    if (!IsDeusEx()) return LoadSaveFile("Save" + std::to_string(slotNum) + "." + GetSaveExtension());
    RefreshSaveInfos();
    auto folder = SaveSlotFolder(slotNum);
    auto* infoPackage = GetSaveInfoPackage(folder);
    if (!infoPackage) return nullptr;
    auto* info = UObject::Cast<UDXSaveInfo>(infoPackage->GetUObject("DeusExSaveInfo", "MyDeusExSaveInfo"));
    if (!info) return nullptr;
    auto map = fs::path(info->MapName());
    if (map.empty() || map != map.filename() || map == "." || map == "..") Exception::Throw("Invalid saved map name");
    return LoadSaveFile((fs::path(folder) / map).string());
}
...
{
	saveInfos.clear();
	if (!fs::is_directory(gameSaveFolderPath)) return;
	for (const auto& entry : fs::directory_iterator{gameSaveFolderPath})
	{
        ...
        if (name != "QuickSave" && (name.size() != 8 || name.substr(0, 4) != "Save" || name.find_first_not_of("0123456789", 4) != std::string::npos)) continue;
