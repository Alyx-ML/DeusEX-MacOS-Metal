...
int PackageWriter::GetObjectReference(UObject* obj)
{
	...
	// Runtime objects (mission flags, notes, conversation history) belong to
    // Transient but are reachable through persistent properties in a saved level.
    if (obj->package == Source || (!isClass && obj->package == Source->GetPackageManager()->GetTransientPackage()))
    {
